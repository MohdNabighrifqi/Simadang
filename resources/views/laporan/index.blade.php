...
<div style="display:flex;gap:8px;flex-wrap:wrap;">
    ...
    <form method="GET" action="{{ route('laporan.index') }}"
          style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin-bottom:1.2rem;">
        <input type="text" name="cari_lokasi" placeholder="Cari lokasi penampakan..."
               class="form-control" style="width:200px;" value="{{ request('cari_lokasi') }}">
        <button type="submit" class="btn btn-gray btn-sm" style="padding:9px 12px;" title="Cari">
            <i class="fa-solid fa-magnifying-glass"></i>
        </button>
        <select name="kondisi" class="form-control" style="width:auto;" onchange="this.form.submit()">
        ...
        <div style="display:flex;gap:4px;align-items:center;">
            <input type="date" name="tanggal_dari" class="form-control" style="width:auto;" title="Dari tanggal"
                   value="{{ request('tanggal_dari') }}" onchange="this.form.submit()">
            <span style="color:var(--text-muted);">—</span>
            <input type="date" name="tanggal_sampai" class="form-control" style="width:auto;" title="Sampai tanggal"
                   value="{{ request('tanggal_sampai') }}" onchange="this.form.submit()">
        </div>
        <a href="{{ route('laporan.index') }}" class="btn btn-gray btn-sm" style="padding:9px 16px;">
